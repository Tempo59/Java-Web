...
SELECT stu.*,sco.* 
FROM tbstudent AS stu INNER JOIN tbsc AS sc INNER JOIN TbCourse AS sco
ON stu.stuid=sc.sid AND sco.cosid=sc.cid

DESC tbsc;
-- 存储过程（带参数（输入参数/输出参数）和不带参数）
DROP PROCEDURE IF EXISTS pro_avg;
-- 传递一个学号，计算该名学生的平均成绩
drop procedure if exists SpGetAvgScoreByStuId;
delimiter $$;
create procedure SpGetAvgScoreByStuId(IN stuId INT, out avgScore float)
begin 
    select avg(score) into avgScore from TbSC where sid=stuId;
end $$;
delimiter ;
SET @stuid = 1001;
CALL SpGetAvgScoreByStuId(@stuid,@score);
SELECT @score;
